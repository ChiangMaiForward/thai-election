const API_URL = "https://script.google.com/macros/s/AKfycbxQ79QlsadIqW12MLP-hoKSBtFQRaXtFeRYGyJkF-_q-UK9qJ4az-0XFOQxVGL9-baeiA/exec"; 
...
function buildPartySummary(){
  ...
  state.party_list.forEach(p=>{
    ...
    map[name] = { 
      party: name, 
      listSeats: seats, 
      conSeats: conCount[name]||0,
      color: p.color || null   // ✅ ใช้สีจาก Google Sheet
    };
  });

  Object.keys(conCount).forEach(p=>{
    if(!map[p]) map[p] = { party: p, listSeats:0, conSeats:conCount[p]||0, color:null };
  });
...
function buildSeatsMapping(){
  ...
  state.constituencies.forEach(c=>{
    seats.push({ 
      type:'con', 
      party: String(c.party1||"Independent"), 
      constituency: c.name, 
      id: c.id 
    });
  });

  while(seats.length < CONSTITUENCY_SEATS) 
    seats.push({ type:'con', party:'Vacant', constituency:null });
  ...
  while(seats.length < TOTAL_SEATS) 
    seats.push({ type:'list', party:'Vacant' });
...
function colorForParty(p){
  ...
  if(found && found.color) return found.color;  // ✅ ใช้สีที่มากับ Google Sheet

  // fallback: generate random but stable color
...
function renderLegend(){
  ...
  state.parties.forEach(p=>{
    ...
    sw.style.background=colorForParty(p.party);
    it.appendChild(sw);
...
function renderSeatMap(){
  ...
  const height=420;
  const cx=width/2;const cy=height-10;
  const radius=Math.min(width/2-20,360);
  const svg=document.createElementNS("http://www.w3.org/2000/svg","svg");
  svg.setAttribute('width',width);svg.setAttribute('height',height);

  const n=state.seatsMapping.length;
  const angleStart=0;        // ✅ โค้งขึ้น
  const angleEnd=Math.PI;    // ✅ โค้งขึ้น

  const rows=10;
  const seatsPerRow=Math.ceil(n/rows);
  ...
  for(let i=0;i<n;i++){
    ...
    const sx=cx+Math.cos(ang)*rowRadius;
    const sy=cy+Math.sin(ang)*rowRadius;
    ...
    circle.setAttribute('fill',colorForParty(seat.party));
...
function showTooltip(e){
  ...
  const seat=state.seatsMapping[idx];
  let text='';
  if(seat.type==='con'){
